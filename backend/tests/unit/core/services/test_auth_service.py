...
async def test_register_user(monkeypatch):
    ...
    monkeypatch.setattr(services_pkg.auth_service, "RefreshTokenRepository", lambda s: FakeTokenRepo())
    ...
    req = RegisterRequest(username="alice", password="Password123!", confirm_password="Password123!", full_name="Alice")
...
async def test_authenticate_user_success(monkeypatch):
    ...
    class FakeUserRepo:
        async def get_by_username(self, username):
            return DummyUser(
                id=user_id, username=username, full_name=None, is_active=True,
...
async def test_refresh_token_flow(monkeypatch):
    ...
    class FakeUserRepo:
        async def get_by_id(self, uid):
            return DummyUser(
                id=uid, username="bob", full_name=None, is_active=True,
                created_at=datetime.now(timezone.utc),
                updated_at=datetime.now(timezone.utc),
            )
...
async def test_change_password_revokes_tokens(monkeypatch):
    ...
    class FakeUserRepo:
        async def get_by_id(self, user_id):
            return DummyUser(id=user_id, username="x", password_hash="hashed", is_active=True,
    ...
    svc = AuthService(session=None)
    ok = await svc.change_password(uid, PasswordChangeRequest(current_password="old", new_password="newPass123!", confirm_new_password="newPass123!"))
    assert ok is True
