...
class FakeNoteRepo:
    ...
    async def get_by_id_and_user(self, nid, uid):
        return (
            self.note if self.note and self.note.id == nid and self.note.owner_id == uid else None
        )
...
async def test_create_and_get_update_delete_and_list(monkeypatch):
    ...
    created = await svc.create_note(
        user_id, Dummy(title="T", content="c #work", is_public=False, hyperlinks=[], tags=["home"])
    )
    ...
    # update
    upd = await svc.update_note(
        note_id,
        user_id,
        Dummy(title="T2", content="new #x", is_public=True, hyperlinks=["http://a"], tags=["y"]),
    )
